...
class DataHandlingKaska:
    """
    Pre-processes the data requested by the user from KaSKA outputs
    """
    def __init__(self, directory):
        ...
        with open(os.path.join(os.path.dirname(__file__),
                               'transform_config_kaska.yaml'), 'r') as t_config:
    ...
    def __build_vrts(self):
        ...
        for param in self.parameters:
            ...
            vrt_filename_warp = f"{self.data_directory}/{param}_warped.vrt"
            ...
            if not os.path.isfile(vrt_filename_warp):
                ...
                # convert to lat/lon
                gd = gdal.Warp(
                    srcDSOrSrcDSTab=f"{self.data_directory}/{param}.tif",
    ...
    def __extract_timesteps_from_gdal(self, param, var):
        """
        Use the Source Filenames from gdal to extract the timestep values.
        :param param:
        :param var:
        :return:
        """
        ...
        # Open the dataset
        gdata = gdal.Open(f"{self.data_directory}/{param}.tif")

        # For each band extract the DoY
        nbands = gdata.RasterCount
        datestrings = []
        for nband in range(nbands):
            band = gdata.GetRasterBand(nband+1)
            md = band.GetMetadata()
            datestrings.append(md['DoY'])
    ...
    def get_available_parameters(self):
        ...
        all_filepaths = sorted(glob.glob(f'{self.data_directory}/*.tif'))

        # Discard all but the filenames
        all_files = [fp.split('/')[-1] for fp in all_filepaths]
        parameters = [osp.splitext(osp.basename(fp))[0] for fp in all_filepaths]
